...
<?php
		...
		while($row = mysqli_fetch_array($result)) {
			echo "<tr>";
			echo "<td>".$row['codmun']."</td>";
			echo "<td>".$row['nombre']."</td>";
			echo "<td class='text-center'><a class='btn btn-info btn-xs' data-toggle='modal' data-target='#squarespaceModal'  onclick='$(\"#codmun\").val(".$row['codmun'].");$(\"#name\").val(\"".$row['nombre']."\");'><span class='glyphicon glyphicon-edit'></span> Editar</a> <a href='municipio.php?codmun=".$row['codmun']."' class='btn btn-danger btn-xs' onclick='return confirm(\"&iquest;Desea eliminar este elemento&#63;\")'><span class='glyphicon glyphicon-remove'></span> Eliminar</a></td>";
			echo "</tr>";
		}	
?>
    </table>
